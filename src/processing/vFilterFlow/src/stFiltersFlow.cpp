...
std::pair<double,double> stFiltersFlow::filtering(int& x,int& y,double& theta,double& time){ //(const emorph::vQueue& subsurf, int& curr_ts, double& theta, double& phase){ {
...
//    }
     ...
     //CHANGED
     double dx_theta =  x * cos(theta) + y * sin(theta);
     double dy_theta = -x * sin(theta) + y * cos(theta);
     ...
     double even_spatial = D * gaussian_spatial  * cos( (2 * M_PI * f1 ));
     double odd_spatial = D * gaussian_spatial * sin( (2 * M_PI * f1));
